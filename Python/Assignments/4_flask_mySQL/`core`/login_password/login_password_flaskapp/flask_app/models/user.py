...
class User:
    DB = "login_reg_users_schema" #which database are you using for this project
    ...
    def validate_user_reg(reg_info):
        ...
        is_valid = True
        if not len(reg_info['first_name']) > 0:
            flash("Please enter your first name.")
            is_valid = False
        if not len(reg_info['last_name']) > 0:
        ...
        for character in reg_info['pw']:
            if character == character.upper():
                is_uppercase = True
            if character == isinstance(character, int):
                is_num = True
        if not is_uppercase:
...
# Staticmethod for login verify, use password from get_user_by_email_pw results. Use `User.method_name` when using Staticmethod
    # LOGIN VALIDATION
    @staticmethod
